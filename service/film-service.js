...
class FilmService {
  ...
  async getFavouriteFilms(user_id, page = 1, limit = 'ALL') {
    ...
    return (await db.query(`SELECT id, title, time FROM favourite_film WHERE user_id = '${user_id}' LIMIT ${limit} OFFSET '${offset}'`));
  }

  async addFavouriteFilm(film_id, user_id, filmTitle) {
    ...
    return (await db.query(`INSERT INTO favourite_film VALUES ('${film_id}', '${filmTitle}', now(), '${user_id}') RETURNING *`))[0];
  }
  ...
  async getLaterFilms(user_id, page = 1, limit = 'ALL') {
    ...
    const offset = limit !== 'ALL' ? (page - 1) * limit : page-1;
    return (await db.query(`SELECT id, title, time FROM later_film WHERE user_id = '${user_id}' LIMIT ${limit} OFFSET '${offset}'`));
  }

  async addLaterFilm(film_id, user_id, filmTitle) {
    const film = await this.isFilmInTheTable(film_id, user_id, this.LATER_FILM);
    if(film) {
      throw ApiError.BadRequest('The film is already in the table')
    }
    return (await db.query(`INSERT INTO later_film VALUES ('${film_id}', '${filmTitle}', now(),  '${user_id}') RETURNING *`))[0];
  }
  ...
  async getRatedFilms(user_id, page = 1, limit = 'ALL') {
    ...
    const offset = limit !== 'ALL' ? (page - 1) * limit : page-1;
    return (await db.query(`SELECT id, title, time, rating FROM rated_film WHERE user_id = '${user_id}' LIMIT ${limit} OFFSET '${offset}'`));
  }

  async addRatedFilm(film_id, rating, user_id, filmTitle) {
    ...
    return (await db.query(`INSERT INTO rated_film VALUES ('${film_id}', '${filmTitle}', now(), '${user_id}', '${rating}') RETURNING *`))[0];
  }
